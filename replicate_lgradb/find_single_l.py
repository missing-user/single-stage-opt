...
def find_regcoil_distance(lcfs):
    ...
    search_initial_points = np.linspace(search_interval[0],search_interval[1], 5)
    kinftys = np.array([kinfty_at_regcoil_distance(float(l)) for l in search_initial_points])
    ...
    opt_result = scipy.optimize.root_scalar(
        lambda l: kinfty_at_regcoil_distance(float(l) - target_k), bracket=search_interval.tolist(),# tol=target_k * 1e-6, method="brentq"
    )
